#!/usr/bin/env python3
"""Verify the source-potential depth-three reciprocal dual exactly."""
...
EXPECTED_BOUNDARY = (
    17669,
    14363663254750611814440,
    618223181506832115,
    157,
    294001978938214713492,
    37,
    1114349115318026335,
)
...
def verify() -> None:
    ...
    for n, label in candidates:
        ...
        source_potential_cost = 2 * (source_label - 1) + 2 * O * delay

        # The least admissible layer minimizes the source-potential cost.
        # Moving to a higher layer adds at least 2O, while the source term can
        # fall by strictly less than 2O.
        assert source_potential_cost >= 0

        combined = 46 * c3 + 6 * source_potential_cost
        items.append((
            n,
            combined,
            label,
    ...
    assert reciprocal_bound < Fraction(85_238_902, 1_000_000_000)
    assert reciprocal_bound >= Fraction(85_238_901, 1_000_000_000)
    ...
    assert residual > Fraction(14_695_304, 1_000_000_000)
    needed = (residual * CUTOFF).numerator // (
        residual * CUTOFF
    ).denominator + 1
    assert needed == 14696

    print("source-potential two-constraint dual verified")
    print(f"survivors={len(candidates)}")
    print(f"h3 maximum={h3_max}, h3 sum={h3_sum}")
    print(f"combined boundary after {full} complete items")
