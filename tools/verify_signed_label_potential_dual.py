#!/usr/bin/env python3
"""Verify the signed-label potential depth-three reciprocal dual exactly."""
...
def verify() -> None:
    ...
    for t in range(1, H + 1):
        ...
        for n in range(rho, CUTOFF + 1, D):
            ...
            # The potential assigns +(s-1) on target labels and -(u-1) on
            # least-source labels.  Disjointness makes this definition valid.
            ...
            # Any higher layer adds 2O.  Its nonnegative source component is
            # at least zero, whereas the least-layer source component is
            # 2*(source_label-1)<2O; hence the least layer strictly minimizes
            # the potential-shifted scalar cost.
    ...
    assert residual > Fraction(14_707_301, 1_000_000_000)
    needed = (residual * CUTOFF).numerator // (
        residual * CUTOFF
    ).denominator + 1
    assert needed == 14708

    print("signed-label potential dual verified")
    print(f"survivors={len(items)}")
    print("least-source and target label sets are disjoint")
