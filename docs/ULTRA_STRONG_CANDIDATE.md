...
The multiplier has the divisor

```text
15099 = 3*7*719.
```

The order of `2` modulo `15099` is exactly

```text
2154 = 2*3*359.
```

Therefore all possible output residues modulo `15099` can be enumerated by the compact finite set

```text
2^(-a) mod 15099,   0<=a<2154.
```

For an even residue, the least odd positive representative is obtained by adding `15099`; for an odd residue it is the residue itself. Exact enumeration of all `2154` classes gives

```text
minimum odd representative other than 1 = 25.
```
...
This residue certificate is independently checked by `verify_output_residue_bound()`.
...
The chosen `B` is maximal for this fixed rational exponent bound: increasing to the next even cycle length makes `r` one larger and causes (5) to fail.
...
2. it enters a nontrivial positive cycle of length greater than `148,557,456,445,856,651,509`.

It cannot return to `1`, and all shorter cycles are excluded by a compact exact certificate. The verifier does not simulate that many steps; it checks a small collection of integer, modular, and rational inequalities.
