...
Thus the exceptional division does not always erase the accumulated gain. There are arbitrarily large complete finite macroblocks whose endpoint is already larger than their start.
...
E_m(L) == u + 2^(m-1)          (mod 2^m),  if k>=1.       (3)
```
...
by the binomial theorem and divide by `2^(m+k)`.

The linear term becomes `u`. If `k=0`, the quadratic and all later terms are divisible by `2^m`.
...
so the quadratic term is exactly `2^(m-1)` times an odd number and is congruent to `2^(m-1)` modulo `2^m`. Every term of degree at least three is divisible by `2^m`. This proves (3). QED.
...
Fix `m` and `k`, and vary the positive odd part `u`. Put
...
The number `c` is odd, and (1) becomes

```text
E_(m,k)(u) = c * (1 + A + ... + A^(u-1)).     (4)
```
...
Indeed, the isometry theorem makes the map on odd residue classes modulo `2^S` injective. The domain and codomain each contain `2^(S-1)` odd classes, so the map is bijective. Therefore `-1` has exactly one preimage.

The classes `u_S` lift uniquely as `S` increases and define one odd 2-adic target `u_*`. This target is not a nonnegative ordinary integer. If it were an ordinary positive integer `U`, then the nonzero positive integer `E_(m,k)(U)+1` would be divisible by every power of two, which is impossible.

So arbitrary finite regeneration is possible, but exact infinite regeneration requires approaching a genuinely nonintegral 2-adic target.
...
It still does **not** prove an infinite divergent positive orbit. The unresolved step is to turn the nested finite regeneration targets into one ordinary positive starting integer, or to find a different certificate whose growth survives indefinitely without converging to a nonintegral 2-adic target.