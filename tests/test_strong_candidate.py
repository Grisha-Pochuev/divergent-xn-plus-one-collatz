...
def test_strong_candidate_cycle_bound_inequalities() -> None:
    ...
    assert details["maximum_even_r"] == 20_750_000
    assert details["maximum_odd_r"] == 20_749_999
    assert 20_750_000 * Y_NUM * EXPONENT_BOUND.denominator < (
        Y_DEN * EXPONENT_BOUND.numerator
    )


def test_rational_exponential_upper_bound() -> None:
    upper = exp_upper_bound(EXPONENT_BOUND)
    headroom = Fraction(3 * (1 << 30), 3 * Q + 1)
    assert upper < headroom < 2


def test_strong_candidate_full_certificate() -> None:
    report = verify_certificate()
    assert report["Q"] == Q
    assert report["excluded_nontrivial_cycle_lengths_through"] == 41_500_000
